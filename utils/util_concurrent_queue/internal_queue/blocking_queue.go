...
/**
 * BlockingQueue is A multi-producer, multi-consumer enqueue
 */

type BlockingQueue struct {
	// The number of items in the EfficientRingbuffer
	count uint64
...
// Pushes the specified element at the tail of the enqueue.
...
// Inserts the specified element at the tail of this enqueue if it is possible to
// do so immediately without exceeding the enqueue's capacity,
// returning true upon success and false if this enqueue is full.
// Does not block the current goroutine
...
// Pops an element from the head of the enqueue.
// Does not block the current goroutine
...
// Just attempts to return the tail element of the enqueue
...
// Clears all the queues elements, cleans up, signals waiters for enqueue is empty
...
// Takes an element from the head of the enqueue.
// It blocks the current goroutine if the enqueue is Empty until notified
func (q *BlockingQueue) Get() (interface{}, error) {
	q.lock.Lock()

	for q.count == 0 {
		// We wait here until the enqueue has an item
		q.notEmpty.Wait()
	}
...
// Puts an element to the tail of the enqueue.
// It blocks the current goroutine if the enqueue is Full until notified
func (q *BlockingQueue) Put(item interface{}) (bool, error) {
	...
	for q.count == q.store.Size() {
		// We wait here until the enqueue has an empty slot
		q.notFull.Wait()
	}
	// Critical section after wait released and predicate is false
